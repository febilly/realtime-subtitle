...
> **2026-09-06 更新：** 字幕展示已与物理音频裁剪分开。内部句界立即分行，句号变逗号时保留分行；默认短停顿更新最小间隔 0.3 秒，连续语音保底更新间隔 4 秒。语义扫描的结果也立即展示，裁剪后的已知尾句直接交接。物理裁剪默认启用，窗口达到约 8 秒后寻找最后一个确认的安全句末，一次可裁掉多句前缀。当前行为与验收口径见 [本地字幕行为](local-subtitle-behavior.md)。下文保留原始物理裁剪实验与历史测量，其中“只扫描不展示”和 opt-in 等描述不再代表当前路径。
