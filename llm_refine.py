...
def _build_tokipona_translate_prompt(source: str, target_lang_value: str, context_block: str) -> str:
    ...
    return (
        ...
        f"{vocab}\n\n"
        "The speaker may also MIX toki pona with words from other languages (English, names, or anything "
        "else), so some tokens are genuinely NOT toki pona and were never meant to be.\n\n"
        "Do TWO things:\n"
        "1. Reconstruct what the speaker actually said, correcting ONLY the toki pona. For each token "
        "decide: was it meant as toki pona, or was it a word from another language / a name that the "
        "speaker code-switched into?\n"
        "   - If it was meant as toki pona, map the garbled STT token to the toki pona word it most likely "
        "was, by sound, using ONLY words from the list above.\n"
        "   - If it was a word from another language or a name, keep it AS-IS in its original form. Do NOT "
        "translate it into toki pona, and do NOT force-map it onto a similar-sounding toki pona word — "
        "leave the foreign/original word exactly as the speaker said it (fixing only obvious STT "
        "mis-spellings of that same word).\n"
        "Keep toki pona grammar words (li, e, la, pi, o, en, ...) and word order. Do not invent meaning "
        "that is not there; if a token is unrecoverable, drop it rather than guessing wildly.\n"
