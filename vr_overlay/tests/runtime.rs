...
use rinbridge_overlay::{
    ...
    CaptionChannel, CaptionRenderer, CaptionUpdate, FakeOpenVr, OpenVrError, OverlayBridgeEvent,
...
#[tokio::test]
async fn runtime_replaces_source_and_translation_lines_independently() {
    let mut runtime = OverlayRuntime::new(OverlayPresentationSnapshot::default());
...
#[tokio::test]
async fn bridge_client_reads_desktop_ws_and_pushes_refinement_immediately() {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let server = tokio::spawn(async move {
        let (stream, _) = listener.accept().await.unwrap();
        ...
        ws.send(Message::Text(
            json!({"type": "ipc_status", "connected": true})
                .to_string()
                .into(),
        ))
        .await
    ...
    let source = client.next_message().await.unwrap();
    assert!(matches!(
        source,
        OverlayBridgeEvent::Captions(update)
            if update.source_updated && !update.translation_updated && update.self_text == "hello"
    ));
    let refinement = client.next_message().await.unwrap();
    assert!(matches!(
        refinement,
        OverlayBridgeEvent::Captions(update)
