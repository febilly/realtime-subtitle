...
use rinbridge_overlay::renderer::LineRole;
#[cfg(windows)]
use rinbridge_overlay::WindowsBundledFontCollection;
...
fn renderer_runtime_bundled_font_path_uses_packaged_app_data_fonts_directory() {
    assert_eq!(
        bundled_font_path_from_exe_dir(Path::new("C:/RinBridge")),
        PathBuf::from("C:/RinBridge")
            .join("rinbridge")
            .join("data")
            .join("fonts")
...
// The Noto CJK TTC asset lives in the upstream RinBridge repo
// (src/rinbridge/data/fonts/), outside this crate; it is not vendored here.
// Rendering falls back to system CJK fonts (verified on real hardware).
#[cfg(windows)]
#[test]
#[ignore = "requires upstream RinBridge font asset not present in vendored copy"]
