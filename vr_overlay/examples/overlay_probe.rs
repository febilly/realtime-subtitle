...
fn probe(api: &VR_IVROverlay_FnTable, key: &str) {
    ...
    let find =
        unsafe { (api.FindOverlay.expect("FindOverlay"))(key_c.as_ptr().cast_mut(), &mut handle) };
    ...
    let tt_err = unsafe {
        (api.GetOverlayTransformType
            .expect("GetOverlayTransformType"))(handle, &mut transform_type)
    };
    ...
    let td_err = unsafe {
        (api.GetOverlayTransformTrackedDeviceRelative
            .expect("GetOverlayTransformTrackedDeviceRelative"))(
            handle, &mut device, &mut matrix
        )
    };
    ...
    let w_err = unsafe {
        (api.GetOverlayWidthInMeters
            .expect("GetOverlayWidthInMeters"))(handle, &mut width)
    };
    ...
    if width > 0 && height > 0 {
        ...
        println!(
            "GetOverlayImageData read -> {out_w}x{out_h} err={data_err} errno={}",
            data_err as i32
        );
